...
class FocusGUIController:
    ...
    def show_unlock_dialog(self):
        ...
        ttk.Label(main_frame, text="Enter the full password (all 3 parts combined):", 
                 font=("Arial", 12, "bold")).pack(pady=(0, 10))
        
        password_entry = ttk.Entry(main_frame, font=("Courier", 12), width=50, show="*")
        password_entry.pack(pady=(0, 20))
        password_entry.focus()
        
        def verify_and_unlock():
            entered_password = password_entry.get()
            if self.agent.verify_password(entered_password):
                dialog.destroy()
                if self.agent.end_session(password_verified=True):
        ...
        ttk.Button(button_frame, text="Unlock", command=verify_and_unlock).pack(side=tk.LEFT, padx=(0, 10))
        ttk.Button(button_frame, text="Cancel", command=dialog.destroy).pack(side=tk.LEFT)
        
        password_entry.bind('<Return>', lambda e: verify_and_unlock())
    ...
    def on_closing(self):
        ...
        if self.agent.session_active:
            messagebox.showwarning("Session Active", 
                                  "Cannot close during an active focus session!\n\n"
                                  "Use the emergency unlock password to end the session early.")
        else:
            self.running = False
            self.root.destroy()
